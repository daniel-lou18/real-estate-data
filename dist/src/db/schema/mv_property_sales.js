...
export const apartments_by_insee_code_month = pgMaterializedView("apartments_by_insee_code_month").as((qb) => qb
    ...
    total_area: sql `round((coalesce(sum(${propertySales.ApartmentFloorArea}), 0))::numeric, 1)`.as("total_area"),
    avg_area: sql `round((coalesce(avg(${propertySales.ApartmentFloorArea}), 0))::numeric, 1)`.as("avg_area"),
    avg_price_m2: sql `round(sum(${propertySales.price}) / nullif(sum(${propertySales.ApartmentFloorArea}), 0))`.as("avg_price_m2"),
    total_apartments: sql `coalesce(sum(${propertySales.nbApartments}), 0)::int`.as("total_apartments"),
    apartment_1_room: sql `coalesce(sum(${propertySales.nbapt1pp}), 0)::int`.as("apartment_1_room"),
    apartment_2_room: sql `coalesce(sum(${propertySales.nbapt2pp}), 0)::int`.as("apartment_2_room"),
    apartment_3_room: sql `coalesce(sum(${propertySales.nbapt3pp}), 0)::int`.as("apartment_3_room"),
    apartment_4_room: sql `coalesce(sum(${propertySales.nbapt4pp}), 0)::int`.as("apartment_4_room"),
    apartment_5_room: sql `coalesce(sum(${propertySales.nbapt5pp}), 0)::int`.as("apartment_5_room"),
    ...
    median_area: sql `round((percentile_cont(0.5) within group (order by ${propertySales.ApartmentFloorArea}))::numeric, 1)`.as("median_area"),
    min_price_m2: sql `round(min(${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0)))`.as("min_price_m2"),
    max_price_m2: sql `round(max(${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0)))`.as("max_price_m2"),
    price_m2_p25: sql `round(percentile_cont(0.25) within group (order by ${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0)))`.as("price_m2_p25"),
    price_m2_p75: sql `round(percentile_cont(0.75) within group (order by ${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0)))`.as("price_m2_p75"),
    price_m2_iqr: sql `round((percentile_cont(0.75) within group (order by ${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0))) - (percentile_cont(0.25) within group (order by ${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0))))`.as("price_m2_iqr"),
    price_m2_stddev: sql `round(stddev_samp(${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0)))`.as("price_m2_stddev"),
})
    .from(propertySales)
...
export const houses_by_insee_code_month = pgMaterializedView("houses_by_insee_code_month").as((qb) => qb
    ...
    total_price: sql `round(coalesce(sum(${propertySales.price}), 0))`.as("total_price"),
    avg_price: sql `round(coalesce(avg(${propertySales.price}), 0))`.as("avg_price"),
    total_area: sql `round((coalesce(sum(${propertySales.HouseFloorArea}), 0))::numeric, 1)`.as("total_area"),
    avg_area: sql `round((coalesce(avg(${propertySales.HouseFloorArea}), 0))::numeric, 1)`.as("avg_area"),
    avg_price_m2: sql `round(sum(${propertySales.price}) / nullif(sum(${propertySales.HouseFloorArea}), 0))`.as("avg_price_m2"),
    total_houses: sql `coalesce(sum(${propertySales.nbHouses}), 0)::int`.as("total_houses"),
    house_1_room: sql `coalesce(sum(${propertySales.nbmai1pp}), 0)::int`.as("house_1_room"),
    house_2_room: sql `coalesce(sum(${propertySales.nbmai2pp}), 0)::int`.as("house_2_room"),
    house_3_room: sql `coalesce(sum(${propertySales.nbmai3pp}), 0)::int`.as("house_3_room"),
    house_4_room: sql `coalesce(sum(${propertySales.nbmai4pp}), 0)::int`.as("house_4_room"),
    house_5_room: sql `coalesce(sum(${propertySales.nbmai5pp}), 0)::int`.as("house_5_room"),
    price_m2_deciles: sql `jsonb_build_array(
    ...
    max_price: sql `round(max(${propertySales.price}))`.as("max_price"),
    median_price: sql `round(percentile_cont(0.5) within group (order by ${propertySales.price}))`.as("median_price"),
    median_area: sql `round((percentile_cont(0.5) within group (order by ${propertySales.HouseFloorArea}))::numeric, 1)`.as("median_area"),
    min_price_m2: sql `round(min(${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0)))`.as("min_price_m2"),
    max_price_m2: sql `round(max(${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0)))`.as("max_price_m2"),
    price_m2_p25: sql `round(percentile_cont(0.25) within group (order by ${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0)))`.as("price_m2_p25"),
    price_m2_p75: sql `round(percentile_cont(0.75) within group (order by ${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0)))`.as("price_m2_p75"),
    price_m2_iqr: sql `round((percentile_cont(0.75) within group (order by ${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0))) - (percentile_cont(0.25) within group (order by ${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0))))`.as("price_m2_iqr"),
    price_m2_stddev: sql `round(stddev_samp(${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0)))`.as("price_m2_stddev"),
})
    .from(propertySales)
    .where(and(inArray(propertySales.propertyTypeLabel, [
    "UNE MAISON",
...
export const apartments_by_insee_code_week = pgMaterializedView("apartments_by_insee_code_week").as((qb) => qb
    .select({
    inseeCode: propertySales.primaryInseeCode,
    iso_year: sql `extract(isoyear from ${propertySales.date})::int`.as("iso_year"),
    iso_week: sql `extract(week from ${propertySales.date})::int`.as("iso_week"),
    total_sales: sql `count(*)::int`.as("total_sales"),
    total_price: sql `round(coalesce(sum(${propertySales.price}), 0))`.as("total_price"),
    avg_price: sql `round(coalesce(avg(${propertySales.price}), 0))`.as("avg_price"),
    total_area: sql `round((coalesce(sum(${propertySales.ApartmentFloorArea}), 0))::numeric, 1)`.as("total_area"),
