...
export const propertySales = pgTable("property_sales", {
    ...
    inseeCodes: jsonb("l_codinsee").$type(),
    // Add generated column
    primaryInseeCode: varchar("primary_insee_code", {
        length: 10,
    }).generatedAlwaysAs(() => sql `l_codinsee->>0`),
    nbsection: smallint("nbsection"),
    sections: jsonb("l_section").$type(),
    primarySection: varchar("primary_section", {
        length: 5,
    }).generatedAlwaysAs(() => sql `l_section->>0`),
    nbpar: smallint("nbpar"),
    lIdpar: jsonb("l_idpar").$type(),
    primaryParcelId: varchar("primary_parcel_id", {
        length: 15,
    }).generatedAlwaysAs(() => sql `l_idpar->>0`),
    nbparmut: smallint("nbparmut"),
    lIdparmut: jsonb("l_idparmut").$type(),
    // Property details
    ...
    lIdlocmut: jsonb("l_idlocmut").$type(),
    ...
    codtypbienIdx: index("idx_property_sales_codtypbien").on(table.propertyTypeCode),
