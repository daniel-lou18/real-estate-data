...
import { db } from "@/db";
import { apartments_by_insee_code_month, apartments_by_section_year, sectionsGeom, } from "@/db/schema";
...
describe("getLegend", () => {
    ...
    it("returns a quantile legend for communes", async () => {
        ...
        const totalBucketCount = result.buckets.reduce((sum, bucket) => sum + bucket.count, 0);
    ...
    it("handles monthly vs yearly data correctly", async () => {
        ...
        expect(yearlyResult.stats.count).toBeGreaterThanOrEqual(monthlyResult.stats.count);
    });
