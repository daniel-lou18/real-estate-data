...
const MetricDelta = z.object({
    ...
    current: NullableNumber.describe("Value in the comparison period (e.g. year N)"),
    delta: NullableNumber.describe("Absolute difference: current - base"),
    pct_change: NullableNumber.describe("Percentage change: 100 * (current - base) / NULLIF(base, 0); null if base is 0 or missing"),
});
// ----------------------------------------------------------------------------
...
export const YearlyDeltasMetrics = z.object({
    ...
    avg_price_m2: MetricDelta.describe("Deltas for SUM(price) / NULLIF(SUM(area), 0) at group level"),
