...
import { db } from "@/db";
import { apartments_by_insee_code_year_deltas, houses_by_insee_code_year_deltas, apartments_by_section_year_deltas, houses_by_section_year_deltas, } from "@/db/schema/mv_property_sales_deltas";
import { APARTMENT_COMPOSITION_FIELDS, HOUSE_COMPOSITION_FIELDS, METRIC_FIELDS, } from "@app/shared";
// ----------------------------------------------------------------------------
 ...
 */
function transformMetricDelta(row, metricName) {
    return {
 ...
 */
function transformCompositionDeltas(row, metrics) {
    const result = {};
 ...
 */
function transformDeltaRow(row, hasSection, hasApartments, hasHouses) {
    const base = {
    ...
    if (hasApartments) {
        base.apartments = transformCompositionDeltas(row, APARTMENT_COMPOSITION_FIELDS);
    }
    if (hasHouses) {
 ...
 */
function getDeltaOrderBy(viewName, sortBy, sortOrder, metric) {
 ...
 */
function addTemporalConditions(conditions, view, params) {
    if (params.year) {
 ...
 */
function addMetricFilterConditions(conditions, params) {
 ...
 */
function buildDeltaWhereConditions(view, params, includeSection = false) {
    const conditions = [];
...
function buildInseeDeltaWhereConditions(view, params) {
    return buildDeltaWhereConditions(view, params, false);
}
function buildSectionDeltaWhereConditions(view, params) {
    return buildDeltaWhereConditions(view, params, true);
}
// ----------------------------------------------------------------------------
// Repositories: Yearly deltas by INSEE code
// ----------------------------------------------------------------------------
export async function getAptsByInseeYearDeltas(params) {
    const whereConditions = buildInseeDeltaWhereConditions(apartments_by_insee_code_year_deltas, params);
    const orderBy = getDeltaOrderBy("apartments_by_insee_code_year_deltas", params.sortBy, params.sortOrder, params.metric);
    ...
    return results.map((row) => transformDeltaRow(row, false, true, false));
}
export async function getHousesByInseeYearDeltas(params) {
    const whereConditions = buildInseeDeltaWhereConditions(houses_by_insee_code_year_deltas, params);
    const orderBy = getDeltaOrderBy("houses_by_insee_code_year_deltas", params.sortBy, params.sortOrder, params.metric);
    const results = await db
        .select()
        ...
        .offset(params.offset ?? 0);
    // Transform flat DB structure to nested Zod schema format
    return results.map((row) => transformDeltaRow(row, false, false, true));
}
// ----------------------------------------------------------------------------
// Repositories: Yearly deltas by section
// ----------------------------------------------------------------------------
export async function getAptsBySectionYearDeltas(params) {
    const whereConditions = buildSectionDeltaWhereConditions(apartments_by_section_year_deltas, params);
    const orderBy = getDeltaOrderBy("apartments_by_section_year_deltas", params.sortBy, params.sortOrder, params.metric);
    const results = await db
        .select()
        ...
        .offset(params.offset ?? 0);
    // Transform flat DB structure to nested Zod schema format
    return results.map((row) => transformDeltaRow(row, true, true, false));
}
export async function getHousesBySectionYearDeltas(params) {
    const whereConditions = buildSectionDeltaWhereConditions(houses_by_section_year_deltas, params);
    const orderBy = getDeltaOrderBy("houses_by_section_year_deltas", params.sortBy, params.sortOrder, params.metric);
    const results = await db
        .select()
        ...
        .offset(params.offset ?? 0);
    // Transform flat DB structure to nested Zod schema format
    return results.map((row) => transformDeltaRow(row, true, false, true));
}
