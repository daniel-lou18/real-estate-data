 ...
 */
function buildAggregateMetrics(areaColumn) {
...
function buildApartmentWhereClause() {
    return and(inArray(propertySales.propertyTypeLabel, [
        "UN APPARTEMENT",
        "DEUX APPARTEMENTS",
...
export const apartments_by_insee_code_month = pgMaterializedView("apartments_by_insee_code_month").as((qb) => qb
    .select({
    inseeCode: sql `${propertySales.primaryInseeCode}`.as("insee_code"),
    year: sql `${propertySales.year}`.as("year"),
    month: sql `${propertySales.month}`.as("month"),
...
export const houses_by_insee_code_week = pgMaterializedView("houses_by_insee_code_week").as((qb) => qb
    ...
    .where(buildHouseWhereClause())
    .groupBy(propertySales.primaryInseeCode, sql `extract(isoyear from ${propertySales.date})::int`, sql `extract(week from ${propertySales.date})::int`));
export const apartments_by_section_month = pgMaterializedView("apartments_by_section_month").as((qb) => qb
    .select({
    inseeCode: sql `${propertySales.primaryInseeCode}`.as("insee_code"),
    section: sql `concat(${propertySales.primaryInseeCode}, "000", ${propertySales.primarySection})`.as("section"),
    year: sql `${propertySales.year}`.as("year"),
    month: sql `${propertySales.month}`.as("month"),
    ...buildAggregateMetrics(propertySales.ApartmentFloorArea),
    ...buildApartmentComposition(),
    ...
    .from(propertySales)
    .where(buildApartmentWhereClause())
    .groupBy(sql `concat(${propertySales.primaryInseeCode}, "000", ${propertySales.primarySection})`.as("section"), propertySales.year, propertySales.month));
export const houses_by_section_month = pgMaterializedView("houses_by_section_month").as((qb) => qb
    .select({
    inseeCode: sql `${propertySales.primaryInseeCode}`.as("insee_code"),
    section: sql `concat(${propertySales.primaryInseeCode}, "000", ${propertySales.primarySection})`.as("section"),
    year: sql `${propertySales.year}`.as("year"),
    ...
    .where(buildHouseWhereClause())
    .groupBy(sql `concat(${propertySales.primaryInseeCode}, "000", ${propertySales.primarySection})`.as("section"), propertySales.year, propertySales.month));
export const apartments_by_section_year = pgMaterializedView("apartments_by_section_year").as((qb) => qb
    .select({
    inseeCode: sql `${propertySales.primaryInseeCode}`.as("insee_code"),
    section: sql `concat(${propertySales.primaryInseeCode}, "000", ${propertySales.primarySection})`.as("section"),
    year: sql `${propertySales.year}`.as("year"),
    ...
    .from(propertySales)
    .where(buildApartmentWhereClause())
    .groupBy(sql `concat(${propertySales.primaryInseeCode}, "000", ${propertySales.primarySection})`.as("section"), propertySales.year));
export const houses_by_section_year = pgMaterializedView("houses_by_section_year").as((qb) => qb
    .select({
    inseeCode: sql `${propertySales.primaryInseeCode}`.as("insee_code"),
    section: sql `concat(${propertySales.primaryInseeCode}, "000", ${propertySales.primarySection})`.as("section"),
    year: sql `${propertySales.year}`.as("year"),
...
export const SelectApartmentsByInseeCodeMonthSchema = createSelectSchema(apartments_by_insee_code_month);
export const SelectHousesByInseeCodeMonthSchema = createSelectSchema(houses_by_insee_code_month);
export const SelectApartmentsByInseeCodeYearSchema = createSelectSchema(apartments_by_insee_code_year);
export const SelectHousesByInseeCodeYearSchema = createSelectSchema(houses_by_insee_code_year);
export const SelectApartmentsByInseeCodeWeekSchema = createSelectSchema(apartments_by_insee_code_week);
export const SelectHousesByInseeCodeWeekSchema = createSelectSchema(houses_by_insee_code_week);
export const SelectApartmentsBySectionMonthSchema = createSelectSchema(apartments_by_section_month);
export const SelectHousesBySectionMonthSchema = createSelectSchema(houses_by_section_month);
export const SelectApartmentsBySectionYearSchema = createSelectSchema(apartments_by_section_year);
export const SelectHousesBySectionYearSchema = createSelectSchema(houses_by_section_year);
