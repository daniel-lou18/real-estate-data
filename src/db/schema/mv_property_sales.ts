...
export const apartments_by_insee_code_month = pgMaterializedView(
  ...
  qb
    .select({
      ...
      total_price: sql<number>`round(coalesce(sum(${propertySales.price}), 0))`,
      avg_price: sql<number>`round(coalesce(avg(${propertySales.price}), 0))`,
      total_area: sql<number>`round(coalesce(sum(${propertySales.ApartmentFloorArea}), 0), 1)`,
      avg_area: sql<number>`round(coalesce(avg(${propertySales.ApartmentFloorArea}), 0), 1)`,
      avg_price_m2: sql<number>`round(sum(${propertySales.price}) / nullif(sum(${propertySales.ApartmentFloorArea}), 0))`,
      total_apartments: sql<number>`coalesce(sum(${propertySales.nbApartments}), 0)::int`,
      apartment_1_room: sql<number>`coalesce(sum(${propertySales.nbapt1pp}), 0)::int`,
      apartment_2_room: sql<number>`coalesce(sum(${propertySales.nbapt2pp}), 0)::int`,
      apartment_3_room: sql<number>`coalesce(sum(${propertySales.nbapt3pp}), 0)::int`,
      apartment_4_room: sql<number>`coalesce(sum(${propertySales.nbapt4pp}), 0)::int`,
      apartment_5_room: sql<number>`coalesce(sum(${propertySales.nbapt5pp}), 0)::int`,
      ...
      min_price: sql<number>`round(min(${propertySales.price}))`,
      max_price: sql<number>`round(max(${propertySales.price}))`,
      median_price: sql<number>`round(percentile_cont(0.5) within group (order by ${propertySales.price}))`,
      median_area: sql<number>`round(percentile_cont(0.5) within group (order by ${propertySales.ApartmentFloorArea}), 1)`,
      min_price_m2: sql<number>`round(min(${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0)))`,
      max_price_m2: sql<number>`round(max(${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0)))`,
      price_m2_p25: sql<number>`round(percentile_cont(0.25) within group (order by ${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0)))`,
      price_m2_p75: sql<number>`round(percentile_cont(0.75) within group (order by ${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0)))`,
      price_m2_iqr: sql<number>`round((percentile_cont(0.75) within group (order by ${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0))) - (percentile_cont(0.25) within group (order by ${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0))))`,
      price_m2_stddev: sql<number>`round(stddev_samp(${propertySales.price} / nullif(${propertySales.ApartmentFloorArea}, 0)))`,
    })
    .from(propertySales)
    .where(
      and(
...
export const houses_by_insee_code_month = pgMaterializedView(
  ...
  qb
    .select({
      ...
      total_price: sql<number>`round(coalesce(sum(${propertySales.price}), 0))`,
      avg_price: sql<number>`round(coalesce(avg(${propertySales.price}), 0))`,
      total_area: sql<number>`round(coalesce(sum(${propertySales.HouseFloorArea}), 0), 1)`,
      avg_area: sql<number>`round(coalesce(avg(${propertySales.HouseFloorArea}), 0), 1)`,
      avg_price_m2: sql<number>`round(sum(${propertySales.price}) / nullif(sum(${propertySales.HouseFloorArea}), 0))`,
      total_houses: sql<number>`coalesce(sum(${propertySales.nbHouses}), 0)::int`,
      house_1_room: sql<number>`coalesce(sum(${propertySales.nbmai1pp}), 0)::int`,
      house_2_room: sql<number>`coalesce(sum(${propertySales.nbmai2pp}), 0)::int`,
      house_3_room: sql<number>`coalesce(sum(${propertySales.nbmai3pp}), 0)::int`,
      house_4_room: sql<number>`coalesce(sum(${propertySales.nbmai4pp}), 0)::int`,
      house_5_room: sql<number>`coalesce(sum(${propertySales.nbmai5pp}), 0)::int`,
      price_m2_deciles: sql<any>`jsonb_build_array(
      ...
      max_price: sql<number>`round(max(${propertySales.price}))`,
      median_price: sql<number>`round(percentile_cont(0.5) within group (order by ${propertySales.price}))`,
      median_area: sql<number>`round(percentile_cont(0.5) within group (order by ${propertySales.HouseFloorArea}), 1)`,
      min_price_m2: sql<number>`round(min(${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0)))`,
      max_price_m2: sql<number>`round(max(${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0)))`,
      price_m2_p25: sql<number>`round(percentile_cont(0.25) within group (order by ${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0)))`,
      price_m2_p75: sql<number>`round(percentile_cont(0.75) within group (order by ${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0)))`,
      price_m2_iqr: sql<number>`round((percentile_cont(0.75) within group (order by ${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0))) - (percentile_cont(0.25) within group (order by ${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0))))`,
      price_m2_stddev: sql<number>`round(stddev_samp(${propertySales.price} / nullif(${propertySales.HouseFloorArea}, 0)))`,
    })
    .from(propertySales)
    .where(
      and(
